...
public class CalcEngine
{
    ...
    public void equals()
    {
        // Apply the previously selected operator (+ or -)
        if(previousOperator == '+') {
        ...
        leftOperand = 0;  // Reset the left operand after the operation.
    }
    ...
    {
        displayValue = 0;
        leftOperand = 0; // Reset left operand to ensure clean state.
        previousOperator = ' '; // Reset operator to prevent unintended behavior.
    }
    ...
    private void applyPreviousOperator()
    {
        if(previousOperator == '+') {
            ...
            // There was no preceding operator, so set the left operand to current display value.
            leftOperand = displayValue;
        }
    }
}
